...
import intercept from "express-mung";
...
class CacheMiddleware {
  ...
  constructor(cacheManager, options = {}) {
    if (!cacheManager) {
      throw new Error(
        "Must be constructed with a cache-manager as the first argument!"
      );
    }
  ...
  attach(app) {
    ...
    // app.use(intercept.write((buffer, encoding, req, res) => this.cacheSet(req.req.cacheKey, buffer)))
    app.use(intercept.json((chunk, req) => this.cacheSet(req.cacheKey, chunk)));
  }
  ...
  async cacheRoute(req, res, next) {
    try {
      if (
        ...
        this.options.routes.find(
          (route) => req.originalUrl.toLowerCase().indexOf(route) !== -1
        ) !== undefined
      ) {
        ...
        if (result) {
          ...
          const hydratedData = await hydrateHandler(
            req,
            res,
            result,
